#!/bin/bash
# 12:00 自動投稿 - Threadsテキストのみ + 5分後に補足コメント
...
echo "" >> "$LOG"
echo "--- 12:00 Threads投稿開始: $(date '+%Y-%m-%d %H:%M:%S') ---" >> "$LOG"

python3 scripts/run.py --platforms th --followup >> "$LOG" 2>&1
EXIT_CODE=$?

if [ $EXIT_CODE -eq 0 ]; then
  echo "--- 12:00 完了（補足コメント含む）: $(date '+%H:%M:%S') (exit: 0) ---" >> "$LOG"
else
  echo "--- 12:00 エラー: $(date '+%H:%M:%S') (exit: $EXIT_CODE) ---" >> "$LOG"
fi
